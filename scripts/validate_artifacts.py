...
def check_generated_csv_outputs(repo_root: Path) -> None:
    ...
    allowed_status = {"RAW", "FILTERED", "READY_TO_GENERATE", "GENERATED", "PUBLISHED"}
    invalid_status = sorted(
        {
            (row.get("status", "") or "").strip().upper()
            for row in ready_rows
            if (row.get("status", "") or "").strip().upper() not in allowed_status
        }
    )
    if invalid_status:
        raise ValueError(f"ready_to_generate.csv has invalid status values: {invalid_status}")
...
def check_master_sources_csv_contract(repo_root: Path) -> None:
    ...
    empty_platform_rows = [
        ...
        if not (row.get("origin_platform", "") or "").strip()
    ]
    if empty_platform_rows:
        print(
            "[warn] master_sources.csv has blank origin_platform on rows: "
            f"{empty_platform_rows[:5]}"
        )
